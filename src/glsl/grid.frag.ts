...
float gridShaping(float offsetComponent, float gridDivisor, float intensity) {
    return
        clamp(
            -abs(
                -mod(
                    offsetComponent * devicePixelRatio * 2.0 + 1.0,
                    (cellSize * gridDivisor) * devicePixelRatio * 2.0
                ) + 1.5
            ) + 1.5,
            0.0,
            intensity
...
void main() {
    ...
    if (showGrid != 0 && (showGrid == 1 || cellSize >= 10.0)) {
        ...
        float x10 = gridShaping(screenCoordOffset.x, 10.0, 0.3);
        float y10 = gridShaping(screenCoordOffset.y, 10.0, 0.3);
    ...
    gl_FragColor = vec4(texture2D(
            state,
            (screenCoordOffset / stateSize) / cellSize
        ).rgb + grid, 1.0
    );
}
`
